module ChefAWSProvisioner
  class Provision < Chef::Application::Client
    ...
    def reconfigure
      ...
      Chef::Config.chef_provisioning['provision'] = begin
        YAML.load(File.open("#{Chef::Config.chef_repo_path}/config/provisioning/#{Chef::Config.environment}/provision.yml"))
      rescue ArgumentError, Errno::ENOENT => e
        Chef::Application.fatal!("Could not parse YAML configuration file: #{e.message}")
      end
    end
    ...
    def run_application
      Chef::Log.info("Starting provisioning of environment #{@environment}.")
      Chef::Config.chef_provisioning['provision'].each do |step, tasks|
        pool = ProvisionWorker.pool(size: tasks.length, args: [Chef::Config.environment, @started_at])
